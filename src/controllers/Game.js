...
export default class Game {
    ...
    async giveTurnTo(players, i) {
        ...
        chooser.to(roomID).emit('choosing', { name: chooser.player.name });
        io.to(player).emit('choosePages', { wikiid: games[roomID].wiki });
        try {
    ...
    onNavigation(data) {
        ...
        if (steps && steps[steps.length - 1] === endPage.title) return;
        steps.push(page);
